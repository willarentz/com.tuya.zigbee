...
class smartplug extends ZigBeeDevice {
  ...
  async onNodeInit({zclNode}) {
    ...
    // Default factor assumes devices reporting centi-kWh (value * 0.01)
    this.energyFactor = 0.01;
    try {
      // Read manufacturer to allow device-specific overrides
    ...
    try {
      // Prefer standard ZCL multiplier/divisor if available
      ...
      // Some Tuya devices do not expose these, fall back to defaults/overrides
    }
    // Manufacturer-specific override: this device reports in Wh, convert to kWh
